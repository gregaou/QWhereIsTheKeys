...
void OjMechantTourne::direction()
{
		...
		if (deplacement)
		{
				qDebug() << "deplacement horraire";
				if (_dx >0) //la creature allait à droite
				{
						qDebug()<< "droite";
						if (hx1 >= px2)//si la creature depasse le point le plus à droite de la plateforme elle descend
							 bas();
						else
								droite(); //sinon elle continue son chemin vers la droite
				}
				else if(_dx <0) //la creature allait à gauche
				{
						qDebug()<< "gauche";
						if (hx2<= px1) //si la creature depasse le point le plus à gauche de la plateforme elle monte
								haut();
						else
				...
				{
						qDebug()<< "descend";
						if(hy1>=py2) //si la creture depasse le point le plus en bas de la plateforme elle part à gauche
								gauche();
						else
				...
				{
						qDebug()<< "monte hy2 = " << hy2 << "; py1 = "<<py1;
						if(hy2 <= py1) //si la creature depasse le point le plus haut, elle vas à droite
								droite();
						else
								haut(); //sinonelle contine à monter

				}
		}
		else
		{
				qDebug() << "deplacement trigo";
				if (_dx < 0) //la creature allait à gauche
				{
						qDebug()<< "gauche";
						if (hx2 <= px1)//si la creature depasse le point le plus à gauche de la plateforme elle descend
							 bas();
						else
							 gauche(); //sinon elle continue son chemin vers la droite
				}
				else if(_dx >0) //la creature allait à droite
				{
						qDebug()<< "droite";
						if (hx1>= px2) //si la creature depasse le point le plus à gauche de la plateforme elle monte
								haut();
						else
				...
				{
						qDebug()<< "monte";
						if(hy2<=py1) //si la creture depasse le point le plus en bas de la plateforme elle part à droite
								gauche();
						else
				...
				{
						qDebug()<< "descend hy2 = " << hy2 << "; py1 = "<<py1;
						if(hy1 >= py2) //si la creature depasse le point le plus haut, elle vas à gauche
								droite();
						else
								bas(); //sinonelle contine à monter
